...
if grep -q "export PYTHONSTARTUP=" ~/.bash_profile; then
    sed -i '' "s|export PYTHONSTARTUP=.*|export PYTHONSTARTUP=\"$PYTHONRC_PATH\"|g" ~/.bash_profile
else
    echo "export PYTHONSTARTUP=\"$PYTHONRC_PATH\"" >>~/.bash_profile
fi
